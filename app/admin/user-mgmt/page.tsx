...
export default function page({ searchParams }: paramsProps) {
    ...
    const fetchUser = async () => {
        try {
            const response = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/user/all" + `?limit=${limit}&offset=${offset}` + (name ? `&search=${name}` : '') + (role ? `&role=${role}` : '') + (divisi ? `&division=${divisi}` : '' ), { headers: {
